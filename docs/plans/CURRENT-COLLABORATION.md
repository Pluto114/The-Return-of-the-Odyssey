...
更新日期：2026-09-15。**当前 main 已整合 D 的单关平台交付，最终版本尚待验收。** D 的提交 `a340ee6` 已在主分支；A 的 `1bfd796` 尚未与 D 的入口整合。不要用 A 的文件直接覆盖 D 的配置、存储和观测接线。
...
| 角色 | 当前成果 | 下一步 |
| --- | --- | --- |
| A | 主线已有网络基础与首关；独立分支新增奖励、Ready/Director、进程内 Resume | 与 D 的配置/Redis/结算/指标整合，补最终终局与协议缺项，牵头 C 跨端兼容收口 |
...
| D | 首关、同源装备表、Bot 多关状态机、真实指标/管理端、Redis/MySQL 模块和装配 | 与 A 接通恢复/终局；真实三关、恢复与持续负载验收，不能以单关结果替代 |
