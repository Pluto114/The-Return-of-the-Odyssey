...
| 渲染管线 P0b（Katana Zero 重构） | ⏳ **待本地构建确认**：`main.cpp` 已改为「可缩放窗口 + `SetExitKey(KEY_NULL)` + RT(960×540) + 双层清屏 + `-540` 翻转 blit + `rlDrawRenderBatchActive()` + 重算 layout」，`client/CMakeLists.txt` 增加 `src/ui/AssetPath.cpp`、`src/ui/PixelFont.cpp` 与 assets post-build 拷贝。会话内已单独编译 `ui/AssetPath.cpp`（MSVC 14.44 `/W4` 无警告）；`main.cpp` 含 raylib/asio/protobuf 依赖，需完整构建。待你复核：窗口缩放是否为整数倍 + 黑边、ESC 是否仍可退出、拉伸时有无拖影/花屏，以及启动日志中的 `viewport`、`asset root`、`HUD font`、`accessibility` 行 |
