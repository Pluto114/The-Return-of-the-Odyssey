...
| HUD 信息架构 P1b-1（Katana Zero 重构） | ✅ 已完成代码侧：`F1` 由"常显诊断列"改为**整屏诊断视图**（SESSION / NETWORK / INPUT / PREDICTION / WORLD / EVENTS 六组，仍为零分配定长缓冲）；竞技场居中放大为 `480×480 @ (240,30)`；新增顶部目标行 `STAGE nn · HOSTILES m`、过渡卡（clear/preparing/failed/closed）、大厅卡（登录/匹配/等待开局 + 房间/Player ID）、断线态（世界压暗 45% + 顶部红色横幅 + `press R to reconnect`）；移除"Phase 1 …"水印与常显 FPS。实测：logic 套件 594 checks / 0 failures；渲染块内 `std::string`/`std::vector` 构造仍为 0。设计依据见 [CLIENT-HUD-DESIGN.md](../../architecture/CLIENT-HUD-DESIGN.md) §3/§4/§8 |
