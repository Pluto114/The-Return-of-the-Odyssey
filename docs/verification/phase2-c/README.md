...
> **交接入口**：无法当面沟通时，请先读 [HANDOFF.md](HANDOFF.md) —— 已完成清单、剩余工作分区、验证命令、下游必须知道的关键事实、撤回方法全在里面。
...
| P3 指标与 `--no-ui`（Katana Zero 重构） | ✅ 已实现并在会话内验证：`BoundedQueue` 新增 `Depth()/MaxDepth()/ResetMaxDepth()`；`NetClient` 新增出站队列瞬时/峰值（Network Thread 通过原子量发布，Main Thread 读取）；`ui/Metrics.h` 提供 `Ema`、`RttEstimator`（α=0.1）、`ServerTickRateEstimator`（Δtick/Δt，**实测 3 ticks/0.1s → 30Hz，而非 10Hz 快照率**）、`PredictionErrorEstimator`（仅快照到达时更新）与 `MetricSeries`；F1 面板新增队列深度、三项指标与 RTT/Tick 折线图；`--no-ui` / `ODYSSEY_UI_OFF=1` 进 `ClientConfig`（非法值报错）并在运行期跳过整个 UI 层。实测：三套件单独编译运行 **core 557 / logic 624 / config 149 checks，全部 0 failures**（`/W4` 无警告）；`main.cpp` 与 `NetClient.cpp` 单独编译 exit 0；`main.cpp` 大括号 257/257、纯 ASCII |
| P3 面板（F2/F3，Katana Zero 重构） | ✅ 已实现代码侧（纯 Raylib，不依赖 rlImGui 审批）：**F2 实体调试**在 RT 内叠加玩家/怪物包围盒、发送中的瞄准锥、权威↔预测误差线、原始快照↔插值连线，并输出一行 `F2 DEBUG players=.. monsters=.. interpDelay=..t snaps=..`；**F3 无障碍菜单**（↑↓ 选择、ENTER/SPACE 切换）三项开关实时生效并**立即写入 `settings.ini`**（新增 `ui/AssetPath.cpp::SaveAccessibility`，写失败只告警）；**ESC 优先级** F3 → F2 → F1 → 退出。实测：`main.cpp` 与 `AssetPath.cpp` 单独编译 exit 0、大括号 244/244、纯 ASCII；logic 套件 589 checks / 0 failures。待本地构建目视 |
...
| Release UI 性能采集与验收方法 | 采集侧：`ODYSSEY_PERF_FRAMES`（1..4096）+ `ODYSSEY_PERF_LOG` + `ODYSSEY_PERF_TRIGGER=immediate\|playing` 触发整帧 CPU 时间采样（`ui/PerfCapture.h`：待命→进场开始计数、**丢弃计数前 10 个预热帧**（含第 0 帧字体图集）、逐帧写 CSV、汇总打印 `main: perf frames=.. mean=.. median=.. p95=.. max=.. ui=on\|off trigger=playing` 后自行退出）。**触发点必须用 `playing`**：与"可否发输入"同一判据，保证两次运行测的是同一类战斗场景而非登录/匹配帧（`immediate` 仅供冒烟，不作验收数字）。一键执行 `scripts/verify/client-release-ui-perf.ps1`：**每次运行自动起一个对位 loadbot**（凭服务器 `odyssey_match_queue_players` 指标或客户端 `match request sent (queued)` 判定客户端已入队）、不重启服务器、拒绝用非 Release 二进制出结论、跑完写 `report.md` + `hardware.md`。**执行结果见下一行；Release 共享预设仍建议 D 补（不再阻塞测量）** |
| **D7 难度 / 全局 Modifier / 关卡摘要（客户端侧）** | ✅ 代码侧已完成，**等服务端接线上线**：新增 `sync/StageSummary.h`（存储 + ASCII 格式化：`add`→有符号增量、`multiply`→百分比（1.2 → `+20%`）、未知 stat/op 保留原值、最多显示 4 条并报 `(+N more)`、空摘要不绘制）+ `network/PayloadCodec.h` 的 `DecodeStageStartedDetail`/`DecodeStageClearedDetail`（固定 8 条上限、溢出计数）。绘制：顶部目标行下 `DIFF n   CLEAR x.xs   ATK +20%, SPD +10%`、过关卡内追加结果行、F1 的 `stage spawned/diff/clear/detail` 行与完整列表。**关键事实（此前判断有误，已核实）**：字段在 `stage.proto` 的域消息 `StageStarted`/`StageCleared`（`MSG_STAGE_STARTED=400` / `MSG_STAGE_CLEARED=401`）上，而服务端 `convert/event.go` **只发**可靠事件 `StageStartedEvent`/`StageClearedEvent`（325/326，仅 `stage_index`+`server_tick`），**400/401 从未发送、字段也未被填值** → 需 A 补发域消息/补字段（详见 [CLIENT-HUD-DESIGN.md](../../architecture/CLIENT-HUD-DESIGN.md) §10）；客户端此时全程显示 `detail=none(400/401)` 且不画任何数字。实测：logic **709 checks** / protocol **158 checks**，均 0 failures |
| **C-c 药水（客户端侧）** | ✅ 已完成客户端半边（2026-09-16）：`input/InputSample.h` 新增 `PotionIntent` 一次性 latch（门控关闭时按键不入队、门控翻转即丢弃，与 C-b 同规矩）→ `PayloadCodec::PlayerInputData::use_potion` 编码 → `main.cpp` 在**真正发出的**那一帧 30Hz `PlayerInput` 上消费（`Q` 键；`--no-ui` 下不失效，因为它属于游戏输入而非 UI）。日志 `potion intent queued` / `potion blocked (<原因>)` / `potion sent seq=..`；F1 显示 `potion sent=.. pending=..`。**权威药水槽/次数/HP 显示仍缺快照字段（A3）**，F1 明确标注 `charges: await A3`；端到端联调待 A 的 `4462d21` 进 main。实测 logic **733 checks** / protocol **161 checks**，0 failures |
| **双向队列深度 EMA 折线图（P3 分期点名项）** | ✅ 已完成：用 `Ema(0.1)` 平滑入站/出站队列深度，**按快照到达采样**（10Hz 稳定时间基，避免按帧采样把 120 点几秒内滚完），F1 新增第三条折线图（cyan=in，amber=out）。同时把三条图像改为**框内标题**并统一内缩绘图区，避免上一个图框的标题压到下一个的曲线 |
| **辅助 UI 命令记录（P3 分期点名项）** | ✅ 已完成客户端侧：`ui/Metrics.h` 新增 raylib-free 的 `CommandCounter`（`Take()` 返回自上次读取以来的命令数，含单测），HUD 文本唯一漏斗 `DrawHudText` 自动计数，F1 显示 `ui text cmds last=.. ema=..`；`--no-ui` 时读数为 0（这本身就是开关生效的证据）。形状命令尚未计入，ImGui 提交耗时随 P2 加入 |
| **Release 构建临时路径（一键）** | ✅ 新增 `scripts/verify/build-client-release.ps1`：按 `client-windows` 预设 1:1 的变量在**独立目录** `build/client-windows-release` 配置 Release（不改 `CMakePresets.json`/`build.ps1`、不碰 Debug 树），并拒绝复用非 Release 的已有缓存。**Release ≤1.5ms 验收因此不再阻塞于 D 的共享预设**（共享预设仍建议由 D 补齐，供发布流程统一使用） |
| **Release UI 性能验收（提示词 §四.3 唯一口径）** | ✅ **已执行并 PASS（2026-09-16）**：[release-ui-perf-2026-09-16-2355/](release-ui-perf-2026-09-16-2355/report.md) —— 3 轮 ×（UI 开 + `--no-ui`），每轮各 600 帧有效；`mean(on)` 0.2355/0.2395/0.2444 ms，`mean(off)` 0.1296/0.0992/0.1220 ms，**UI 每帧增量中位数 0.1224 ms ≤ 1.5 ms 预算**，轮间离散 0.0344 ms（远小于预算 30%）→ 判定 **PASS 且稳定**。场景为真实战斗（`match ready room=1 teammates=1` → `stage started stage=1` → `stage cleared`，bot 六次全部 `succeeded=1 failed=0`），视口 1920×1080 scale=2。方法：`scripts/verify/build-client-release.ps1` 建 Release 树 + `scripts/verify/client-release-ui-perf.ps1`（每次运行自动起一个 loadbot，凭服务器 `odyssey_match_queue_players` 判断客户端已入队）。**注意**：HUD 字体文件缺失，两次运行都用 raylib 默认字体（对差值无影响）；max 值 8.4–9.6 ms 属孤立单帧抖动（p95 仅 0.12–0.37 ms）。**测试机说明已填完**（[hardware.md](release-ui-perf-2026-09-16-2355/hardware.md)）：内存 15.7 GB、双 GPU、三星 NVMe、Balanced 电源方案 + 插电；其中显示器几何为推断、实际渲染 GPU 未记录，**操作者已于 2026-09-16 确认接受该两项**，记录中无悬空待办 |
| **逐状态截图证据链** | ⚠️ **未执行（如实记录，未伪造）**。定稿 §五.2 要求 `disconnected` / `login` / `playing` / `reward` / `debug` 五个状态的统一分辨率截图。**未就绪原因**：本次交付窗口内优先完成了实现与性能验收（唯一硬指标）；联调环境实际可用（已实测能进 `playing`），因此这是**排期取舍**而非环境不可用 —— `disconnected`/`login`/`playing`/`debug` 四张随时可补，`reward` 需等 A 的奖励路由进 main。按定稿"环境不可用时必须注明未执行+原因、严禁伪造"的规则登记在此 |
