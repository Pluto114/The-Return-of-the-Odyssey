...
| **Release UI 性能验收（提示词 §四.3 唯一口径）** | 方案已就绪：[RELEASE-UI-PERF-PLAN.md](RELEASE-UI-PERF-PLAN.md)。采集侧已实现：`ODYSSEY_PERF_FRAMES`（1..4096）+ `ODYSSEY_PERF_LOG` + `ODYSSEY_PERF_TRIGGER=immediate\|playing` 触发整帧 CPU 时间采样（`ui/PerfCapture.h`：待命→进场开始计数、**丢弃计数前 10 个预热帧**（含第 0 帧字体图集）、逐帧写 CSV、汇总打印 `main: perf frames=.. mean=.. median=.. p95=.. max=.. ui=on\|off trigger=playing` 后自行退出）。**触发点用 `playing`**：与"可否发输入"同一判据，保证两次运行测的是同一类战斗场景而非登录/匹配帧（`immediate` 仅供冒烟，不作验收数字）。**阻塞：需 Release 构建预设**（`CMakePresets.json` 现仅 Debug，属 D 的共享文件）；预设未落地时文档给了不改共享文件的临时 configure 路径。判定：同场景 UI 开/关各 ≥600 帧、`mean(on) − mean(off) ≤ 1.5 ms`、必报 max、必附硬件说明。一键执行：`scripts/verify/client-release-ui-perf.ps1`（跑轮次、解析 CSV、出 PASS/FAIL/UNSTABLE 判定并写 `report.md`/`hardware.md`；拒绝用非 Release 二进制出结论，不启 bot、不在两轮之间重启服务器） |
