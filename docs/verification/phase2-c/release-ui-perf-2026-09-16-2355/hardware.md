...
list. Two values cannot be read that way at all; they are marked as inferred / not recorded and
were **accepted as-is by the operator on 2026-09-16**, so nothing in this table is pending.
...
| GPU / driver | Two adapters present: **Intel(R) UHD Graphics** (`32.0.101.6556`, 2025-01-23) and **NVIDIA GeForce RTX 4060 Laptop GPU** (`32.0.16.1664`). Which one rendered the measured window is not recorded — driver-level per-application assignment decides it, and the criterion compares the same build against itself, so the adapter does not affect the verdict. **Accepted as-is (2026-09-16)** |
...
- Confirmation: the operator reviewed this table and **accepted the inferred display geometry
  and the unrecorded GPU assignment** on 2026-09-16, so the acceptance record is complete with
  no open questions on the machine description.
