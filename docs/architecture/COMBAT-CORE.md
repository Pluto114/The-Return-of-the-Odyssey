...
演示使用一个玩家和 `NewFirstStagePlan` 生成的八只怪物，自动产生瞄准/射击输入；通过同一个 World.Step 执行真实战斗逻辑。固定位置的离线射手会关闭掩体碰撞，以专注验证伤害和清场。
当前固定场景在第 119 Tick 清场：玩家 HP=100，击杀 8，命中 16，开火 20。
...
| 怪物 | 10Hz 按距离和当前目标负载重新选择存活玩家，等距时选较小 Player ID；30Hz 移动并绕开掩体、按攻击距离和 Tick 冷却近战；目标断开/死亡后换目标 |
...
首关生成器使用进程内私有的 SplitMix64 序列，从地图边缘的八个相对锚点中按 Seed 选择并排序八处出生点；不使用全局随机源。相同配置和 Seed 会得到逐字段一致的 Plan，不同房间可用 room_id 等服务器可信值派生 Seed。
...
| `game.NewFirstStagePlan(config, seed)` | B 提供的首关计划入口；传入 `room.Config.World` 和服务端 Seed，返回经过 `ValidateStage` 的八怪 Plan；怪物容量小于 8 时明确报错 |
