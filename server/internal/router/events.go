...
// run it in its own goroutine. stageIndex is sourced from the room's latest
// snapshot each batch, so stage-scoped events carry the current stage index.
func (d *EventDispatcher) Run(rm *room.Room) {
	for batch := range rm.Events() {
		stageIndex := rm.LatestSnapshot().Stage.Index
		d.Dispatch(batch, stageIndex)
	}
}

// Dispatch fans a single event batch out to its subscribers. It is separated
// from Run for testability. stageIndex supplies the stage index for
// stage-scoped events (see convert.Event).
func (d *EventDispatcher) Dispatch(batch game.EventBatch, stageIndex uint32) {
	for _, e := range batch.Events {
		mt, msg, err := convert.Event(e, stageIndex)
		if err != nil {
