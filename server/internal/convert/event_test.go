...
func TestEventStageStarted(t *testing.T) {
	mt, msg, err := Event(game.Event{Kind: game.StageStarted, ServerTick: 7}, 2)
	if err != nil {
...
func TestEventStageLifecycle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		gotMT, msg, err := Event(game.Event{Kind: tc.kind, ServerTick: 8}, tc.stageIdx)
		if err != nil {
...
func TestEventUnknownKind(t *testing.T) {
	_, _, err := Event(game.Event{Kind: game.EventKind(255)}, 0)
	if err == nil {
