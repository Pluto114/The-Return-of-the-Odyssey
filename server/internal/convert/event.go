...
// MessageType field.
//
// stageIndex supplies the stage index for stage-scoped events (StageStarted,
// StageCleared, TeamDefeated). B's game.Event struct does not carry the stage
// index (the room's authoritative stage.View does), so the dispatcher reads it
// from room.LatestSnapshot().Stage.Index and passes it here. It is ignored for
...
func Event(e game.Event, stageIndex uint32) (uint16, proto.Message, error) {
	switch e.Kind {
