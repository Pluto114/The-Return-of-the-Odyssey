...
// display table (see ParseEquipmentTable) generated from the versioned repository
// config at build time. Missing entries degrade to "equipment#<id>".
#pragma once
...
struct EquipmentDisplay {
    ...
    std::string name;
    std::string slot;
    std::string description;  // the catalog's own display text, e.g. "Attack +5"
};

using EquipmentTable = std::map<std::uint32_t, EquipmentDisplay>;

// Parses tab-separated "id, name, slot, description" rows generated by CMake from
// data/equipment/catalog.json. '#' starts a comment; blank lines are ignored. A row
// must have exactly four fields: the catalog validates that display text contains
// no tabs or newlines, so a malformed row is skipped rather than guessed at.
